# Persönliches Coaching
Massgeschneiderte Unterstützung für Ihre persönliche Vorbereitung auf die nächste IT-Zertifizierung oder für spezifische berufliche Herausforderungen.

## Ein paar Infos zu mir
Tom Wechsler 👉 [LinkedIn-Profil](https://www.linkedin.com/in/tom-wechsler)

## Bei welchen Themen kann ich Sie unterstützen?
- Microsoft Azure Zertifizierungen (z.B. AZ-900, AZ-104, AZ-140, AZ-500, AZ-700, AZ-800, AZ-801)
- Microsoft Security Zertifizierungen (z.B. SC-900, SC-200, SC-300, SC-401, SC-100)
- Microsoft 365 Zertifizierungen (z.B. MS-900, MD-102, MS-102, PL-900)
- Github Zertifizierungen (z.B. GH-100, GH-200, GH-500)
- ISC2 Zertifizierungen (z.B. CISSP, SSCP)
- CompTIA Zertifizierungen (z.B. Security+, CySA+, PenTest+, SecurityX)
- Red Hat Zertifizierungen (z.B. RHCSA, RHCE)
- Linux Foundation Zertifizierungen (z.B. LFCS, LFCE)
- Cisco Zertifizierungen (z.B. CCNA CyberOps, CCNA Security, CCNA Routing & Switching)
- Linux Professional Institute Zertifizierungen (z.B. LPIC-1, LPIC-2)
- EC-Council Zertifizierungen (z.B. Certified Ethical Hacker (CEH))
- Ihre Zertifizierung oder Ihr Thema ist nicht aufgeführt? Kontaktieren Sie mich gerne, um Ihre individuellen Bedürfnisse zu besprechen.

## Coaching Angebot
- ⏰ **Dauer:** Flexibel, je nach Bedarf (z.B. 1-2 Stunden pro Sitzung, max 3 Stunden pro Sitzung)
- 💸 **Preis pro Stunde:** Euro 200.- (für Anmeldungen aus der Schweiz – CHF 200.- exkl. MwSt.)
- 🗓️ **Preisstaffelung:** 2 Stunden Euro 350.- / 3 Stunden Euro 450.- (für Anmeldungen aus der Schweiz sind die Preise exkl. MwSt.)
- ➡️ **Individuell abgestimmte Coaching-Sitzungen basierend auf Ihren Bedürfnissen und Zielen.**
- ➡️ **Plattform:** Microsoft Teams (Online-Coaching)

## Was ist im Coaching enthalten?
- Persönliche Beratung und Unterstützung bei der Vorbereitung auf Ihre IT-Zertifizierung.
- Klärung von Fragen und Unsicherheiten zu spezifischen Themen.
- Zugang zu zusätzlichen Ressourcen und Materialien, die Ihnen bei der Vorbereitung helfen.
- Kurze Zusammenfassung der jeweiligen Sessions (z.B. erwähnte Links, Hinweise auf wichtige Punkte/Themen, etc.) - wird Ihnen nach jeder Sitzung per E-Mail zugeschickt.

## Anmeldung
Schreiben Sie eine E-Mail an [training@videotrainer.ch](mailto:training@videotrainer.ch) mit dem Betreff: *„Persönliches Coaching“* und geben Sie bitte an:
- Ihre bevorzugten Coaching-Themen oder Zertifizierungen
- Persönlichen Angaben (Name, Firma, Position und Rechnungsadresse)
- Ihre bevorzugten Nachmittage und Zeiten für die Sitzungen

Ich werde mich dann mit Ihnen in Verbindung setzen, um die Details zu besprechen und die ersten Coaching-Sitzungen zu planen.

---
Ich freue mich darauf, Sie auf Ihrem Weg zu unterstützen!